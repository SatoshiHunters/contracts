...
// MasterChefV2 is the master of Satoshi. He can make Satoshi and he is a fair guy.
//
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 satoshiReward = multiplier.mul(satoshiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        satoshi.mint(devaddr, satoshiReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                safeSatoshiTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function safeSatoshiTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > satoshiBal) {
            transferSuccess = satoshi.transfer(_to, satoshiBal);
        } else {
            transferSuccess = satoshi.transfer(_to, _amount);
        }
		
        require(transferSuccess, "safeSatoshiTransfer: transfer failed");
    }
    ...
    //Satoshi has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _satoshiPerBlock) public onlyOwner {
        massUpdatePools();
        satoshiPerBlock = _satoshiPerBlock;
		
		emit UpdateEmissionRate(msg.sender, _satoshiPerBlock);
    }
}
